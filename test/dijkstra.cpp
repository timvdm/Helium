...
void test_with_atom_predicate()
{
  ...
  Dijkstra<HeMol> d(mol, get_atom(mol, 0), DijkstraExcludeIndex4Atoms());
  ...
  COMPARE(2, d.distance(get_atom(mol, 6)));
  COMPARE(3, d.distance(get_atom(mol, 7)));
  COMPARE(4, d.distance(get_atom(mol, 8)));
  ...
  COMPARE(6, get_index(mol, path[2]));
  COMPARE(7, get_index(mol, path[3]));
  COMPARE(8, get_index(mol, path[4]));
}

int main()
{
  test_all_atoms();
  test_with_atom_predicate();
}
